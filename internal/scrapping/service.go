...
func (s *Service) Configure(conf *Conf, confDb *database.Conf) {
	...
	s.Load.Do(func() {
		...
		// Настраиваем скрапинг с начальной задержкой 30 секунд
		_, err := s.cron.Every(conf.ScrapCron).Minutes().StartAt(time.Now().Add(30 * time.Second)).Do(s.scrap)
		if err != nil {
...
func (s *Service) scrap() {
	...
	if err != nil {
		if repository.IsNotFoundError(err) {
			lastArticle = lastArticleSite - 1000
		} else {
...
func getLastArticle() (int64, error) {
	...
	log.Info().Msgf("WE ARE HERE: PROBLEM IS HERE!")

    // Выполняем HTTP-запрос
    resp, err := http.Get("https://habr.com/ru/articles/")
    if err != nil {
        log.Info().Msgf("Ошибка при выполнении запроса: %v", err)
    ...
    if resp.StatusCode != http.StatusOK {
        log.Info().Msgf("Ошибка: статус ответа %d", resp.StatusCode)
        return 0, fmt.Errorf("неверный статус ответа: %d", resp.StatusCode)
    }

    // Создаем документ goquery из тела ответа
    doc, err := goquery.NewDocumentFromReader(resp.Body)
    if err != nil {
        log.Info().Msgf("Ошибка при создании документа goquery: %v", err)
        return 0, fmt.Errorf("ошибка создания документа goquery: %w", err)
    }
    ...
    // Ищем первый элемент и проверяем его
    selection := doc.Find("article.tm-articles-list__item").First()
    if selection.Length() == 0 {
        return 0, errors.New("no article found with class tm-articles-list__item")
    }

    selection.Each(func(i int, item *goquery.Selection) {
        val, ok := item.Attr("id")
		log.Info().Msgf("Извлеченный ID: %s, OK: %v", val, ok)
        if !ok {
...
func (s *Service) getArticle(id int64) (*external.Article, error) {
    client := &http.Client{
        ...
        CheckRedirect: func(req *http.Request, via []*http.Request) error {
            log.Info().Msgf("WE ARE HERE: Get %q: redirected", req.URL.String())
            // Пропускаем, если редирект на /ru/companies/ или /ru/news/
            if strings.Contains(req.URL.Path, "/ru/companies/") || strings.Contains(req.URL.Path, "/ru/news/") {
                return fmt.Errorf("редирект на неподдерживаемый URL: %s", req.URL.String())
            }
            if len(via) >= 10 {
    ...
    if resp.StatusCode != http.StatusOK {
        return nil, fmt.Errorf("not received 200 status code: %v, url: %v", resp.StatusCode, id)
    }

    doc, err := goquery.NewDocumentFromReader(resp.Body)
    if err != nil {
        return nil, fmt.Errorf("ошибка создания документа goquery: %w", err)
    }
...
func (s *Service) GetArticle(w http.ResponseWriter, r *http.Request) {
    log.Info().Str("module", s.Name).Msg("GetArticle started")

    w.Header().Set("Content-Type", "application/json;charset=utf-8")
    log.Info().Str("module", s.Name).Msg("Set Content-Type header to application/json")

    idStr := chi.URLParam(r, "id")
    log.Info().Str("module", s.Name).Msgf("Extracted ID from URL: %s", idStr)

    id, err := strconv.Atoi(idStr)
    if err != nil {
        log.Error().Str("module", s.Name).Msgf("Failed to parse ID: %v", err)
        w.WriteHeader(http.StatusBadRequest)
        w.Write([]byte(err.Error()))
        return
    }
    log.Info().Str("module", s.Name).Msgf("Successfully parsed ID: %d", id)

    dbArticle, err := s.repo.GetArticleById(r.Context(), id)
    if err != nil {
        if repository.IsNotFoundError(err) {
            log.Info().Str("module", s.Name).Msgf("Article with ID %d not found", id)
            http.NotFound(w, r)
        } else {
            log.Error().Str("module", s.Name).Msgf("Error retrieving article with ID %d: %v", id, err)
            w.WriteHeader(http.StatusInternalServerError)
            w.Write([]byte(err.Error()))
        }
        return
    }
    log.Info().Str("module", s.Name).Msgf("Successfully retrieved article with ID %d", id)

    var tags []string
    err = json.Unmarshal(dbArticle.Tags, &tags)
    if err != nil {
        log.Error().Str("module", s.Name).Msgf("Failed to unmarshal tags for article ID %d: %v", id, err)
        w.WriteHeader(http.StatusInternalServerError)
        w.Write([]byte(err.Error()))
        return
    }
    log.Info().Str("module", s.Name).Msgf("Successfully unmarshaled tags for article ID %d: %v", id, tags)
    ...
    log.Info().Str("module", s.Name).Msgf("Created external.Article for ID %d", id)

    response, err := json.Marshal(article)
    if err != nil {
        log.Error().Str("module", s.Name).Msgf("Failed to marshal article ID %d: %v", id, err)
        w.WriteHeader(http.StatusInternalServerError)
        w.Write([]byte(err.Error()))
        return
    }
    log.Info().Str("module", s.Name).Msgf("Successfully marshaled article ID %d to JSON", id)

    w.WriteHeader(http.StatusOK)
    w.Write(response)
    log.Info().Str("module", s.Name).Msgf("Successfully sent response for article ID %d", id)
}
